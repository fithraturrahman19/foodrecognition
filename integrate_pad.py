...
def browse_file():
    global browse_img
    global path3
    # path3 = filedialog.askopenfilename(initial dir="/Learning/Python", title="Select A File", filetype=
    # (("jpeg files", "*.jpg"), ("all files", "*.*")))
    path3 = filedialog.askopenfilename(initialdir="/Users/ainueo/OneDrive - 서울과학기술대학교/MV Project/test_images",
                                       title="Select A File", filetypes=(("jpeg files", "*.jpg"),
                                                                         ("jpeg files", "*.jpeg"),
                                                                         ("all files", "*.*")))
    # browse_img = Image.open(path3)
    # browse_img = crop_max_square(browse_img)
    ...
    browse_img = ImageTk.PhotoImage(new_im)
    canvasImage.itemconfig(image_id, image=browse_img)
